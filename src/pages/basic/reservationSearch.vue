        ...
        <el-row>
            <el-col :span="25" class="toolbar">
                <el-form :inline="true" :model="filters">
                    ...
                    <el-form-item>
                        <el-date-picker type="daterange" placeholder="选择日期范围" v-model="filters.dateRange" @change="handleDateRangeChange" format="yyyy-MM-dd" :editable="false">
                        </el-date-picker>
                    </el-form-item>
                    <el-form-item>
...
export default {
    ...
    methods: {
        ...
        handleCommand() {
            let downLink = document.getElementById("down-link"),beginDate,endDate;
            try {
                beginDate = new Date(this.filters.dateRange[0]).Format("yyyy-MM-dd");
                endDate = new Date(this.filters.dateRange[1]).Format("yyyy-MM-dd");
            }catch(e){
            ...
            downLink.setAttribute("href", sessionStorage.getItem('baseUrl') + "/sc/exam/exportExamAppointmentInfo?keyWord=" + encodeURI(this.filters.keyWord) + "&examCarTypeName=" + (this.filters.curCarType === "" ? '' : this.filters.curCarType) + "&examStageName=" + encodeURI(this.filters.curState) + "&schoolCode=" + this.schoolCode + "&examBeginDate=" + beginDate + "&examEndDate=" + endDate)
            downLink.click();
        },
